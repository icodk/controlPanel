...
static void drawFrmProcess(lv_obj_t* win) {
    ...
    //int i = 0;
        ...
        lv_obj_align(panel, LV_ALIGN_TOP_LEFT,5, 25);
       ...
       // lv_obj_add_event_cb(panel, config_init, LV_EVENT_CLICKED,(void *) i);
        ...
        lv_label_set_text_fmt(label, "%s  ",get_text(T_TEMP_C));
        lv_obj_align(label, LV_ALIGN_LEFT_MID, 5, 0);
        lv_obj_add_style(label, get_style_label(), 0);

        label = lv_label_create(panel);
        lv_obj_align(label, LV_ALIGN_RIGHT_MID, -10, 0);
        lv_obj_add_style(label, get_style_data(), 0);
        lv_label_set_text_fmt(label, " %s  ", "56.75");


        
        // Setpoint
        panel = lv_obj_create(wcont);
        lv_obj_set_size(panel, 250, 40);
        lv_obj_align(panel, LV_ALIGN_TOP_LEFT, 5, 75);
        lv_obj_add_style(panel, get_style_panel(), 0);
        //lv_obj_add_event_cb(panel, resetMsgBox, LV_EVENT_CLICKED, i);
       // lv_obj_add_event_cb(panel, config_init, LV_EVENT_CLICKED,(void *) i);
       ...
       label = lv_label_create(panel);
        lv_label_set_text_fmt(label, "%s  ", get_text(T_SETPOINT_C));
        lv_obj_align(label, LV_ALIGN_LEFT_MID, 5, 0);
        lv_obj_add_style(label, get_style_label(), 0);
        ...
        lv_obj_align(label, LV_ALIGN_RIGHT_MID, -10, 0);
        lv_obj_add_style(label, get_style_data(), 0);
        lv_label_set_text_fmt(label, " %s  ", "97.75");

        // Error
        panel = lv_obj_create(wcont);
        lv_obj_set_size(panel, 250, 40);
        lv_obj_align(panel, LV_ALIGN_TOP_LEFT, 5, 125);
        lv_obj_add_style(panel, get_style_panel(), 0);

        label = lv_label_create(panel);
        lv_label_set_text_fmt(label, "%s  ", get_text(T_ERROR_C));
        lv_obj_align(label, LV_ALIGN_LEFT_MID, 5, 0);
        lv_obj_add_style(label, get_style_label(), 0);
